<?php
    ...
    // CMS Control 
    // data_1_002903();
    function read_table_data_1_002903($AUTH_MAS,$VALUE){
        // ------------------------------------------------------------------------- INITIALIZE
            $isi = '';
            
            if(!is_null($VALUE))
                {$isi_model = data_002903::read_data_order_by_desc_and_value($VALUE);}
            elseif(is_null($VALUE))
                {$isi_model = data_002903::read_data_order_by_desc();}
            ...
            if(count($isi_model) > 0){
                foreach ($isi_model as $row) {
                    $isi .= '<tr>';
                        ...
                        $isi .= td_me($row->nama);
                        $isi .= td_class_me('text-center',generate_dropdown_button($AUTH_MAS,'002903',$row->id));      
                    $isi .= '</tr>';
                }
