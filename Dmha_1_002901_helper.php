<?php
    ...
    // CMS Control 
    // data_1_002901();
    function read_table_data_1_002901($AUTH_MAS,$VALUE){
        // ------------------------------------------------------------------------- INITIALIZE
            $isi = '';
            
            if(!is_null($VALUE))
                {$isi_model = data_002901::read_data_order_by_desc_and_value($VALUE,1);}
            elseif(is_null($VALUE))
                {$isi_model = data_002901::read_data_order_by_desc(1);}
            ...
            if(count($isi_model) > 0){
                foreach ($isi_model as $row) {
                    $isi .= '<tr>';
                        ...
                        $isi .= td_class_me('text-center',generate_dropdown_button($AUTH_MAS,'002901',$row->id));      
                    $isi .= '</tr>';
                }
            }else{
                $isi .= '<tr>';
