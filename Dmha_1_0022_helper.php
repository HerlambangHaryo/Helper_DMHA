<?php
    ...
    // CMS Control 
    // data_1_002917();
    function read_table_data_1_0022($AUTH_MAS,$PARENT_ID,$VALUE,$SCREEN_WIDTH)
    {
        // ------------------------------------------------------------------------- INITIALIZE
            $isi = '';
            ...
            if(!is_null($VALUE))
                {$isi_model = data_0022::read_data_order_by_desc_and_value($PARENT_ID,$VALUE);}
            elseif(is_null($VALUE))
                {$isi_model = data_0022::read_data_order_by_desc($PARENT_ID);}
            ...
            if(count($isi_model) > 0)
            {
                foreach ($isi_model as $row) 
                {
                    $isi .= '<tr>';
                        ...
                        $isi .= td_class_me('text-center', $row->deskripsi);
                        $isi .= td_class_me('text-center', '');
                        $isi .= td_class_me('text-center', '');
    ...
    function data_0022_isi($LINK)
    {
        ...
        // ------------------------------------------------------------------------- ACTION
            ...
            $isi_model = data_0022::read_data_order_by_desc($INIT_param_1->parent_id);
